package com.ctrip.ibu.flight.internalws.web;
...
public class WebStarter {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Desktop.getDesktop().browse(new URI(String.format("http://127.0.0.1:%s", port)));
  ...
  private static String getWorkingDir() throws Exception {
    URL url = WebStarter.class.getClassLoader().getResource(APP_PROPERTIES);
    if (url != null) {
  ...
  private static int getPort() throws Exception {
    ...
    try {
      prop.load(WebStarter.class.getClassLoader().getResourceAsStream(APPLICATION_PROPERTIES));
