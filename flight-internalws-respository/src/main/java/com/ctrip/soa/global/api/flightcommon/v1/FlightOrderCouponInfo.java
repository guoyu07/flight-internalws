...
public class FlightOrderCouponInfo  {
    ...
    @FieldDoc("是否需要API check 优惠券金额（APP 不用关心）")
    @JsonProperty("IsNeedCheck")
    @XmlElement(name = "IsNeedCheck")
    private boolean isNeedCheck;

    @FieldDoc("是否需要API获取优惠券详情（APP 不用关心，仅用于新优惠券接口,仅需要客户端传CouponCode,此字端为true时IsNeedCheck 不再生效）")
    ...
    /**
     * 是否需要API获取优惠券详情（APP 不用关心，仅用于新优惠券接口,仅需要客户端传CouponCode,此字端为true时IsNeedCheck 不再生效）
     */
